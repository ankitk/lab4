...
def find_ball(opencv_image, debug=False):
	...
	ball = None
	opencv_image = cv2.GaussianBlur(opencv_image, (5,5), 100 )
	circles = cv2.HoughCircles(opencv_image, cv2.HOUGH_GRADIENT, .5, 500, param1=50,param2=34)
	
	if circles is not None:
		if debug == True and len(circles) > 1:
			display_fail(opencv_image)
		ball = circles[0][0]
	
	return ball
	
def display_fail(opencv_image):
	ball = None
	opencv_image = cv2.GaussianBlur(opencv_image, (5,5), 100 )
	circles = cv2.HoughCircles(opencv_image, cv2.HOUGH_GRADIENT, .5, 500, param1=50,param2=34)
	...
	if circles is not None:
		for c in circles:
			display_circles(opencv_image, c)
	else:
